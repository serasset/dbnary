...
public class FrenchExtractorWikiModel extends DbnaryWikiModel {
  ...
  public void getPerson(HashSet<PropertyObjectPair> infos, String person, int rowNumber,
      int rowCount) {
    switch (person) {
      case "j’":
      case "que j’":
        ...
        if ((person.equals("") || person.charAt(0) == '-') && rowCount == 4) {
          ...
          switch (rowNumber) {
            ...
            case 3:
              ...
              return;
            default:
              log.debug("BUG: unexpected row number '" + rowNumber
                  + "' while parsing imperative table for '" + delegate.currentLexEntry() + "'");
              return;
          }
        } else {
