...
public class WiktionaryExtractor extends AbstractWiktionaryExtractor {
 ...
 //        posMarkers.put("ことわざ", "Proverb");
         ...
         nymMarkerToNymName.put("類義語", "syn");
...
//    private HashSet<String> unsupportedSections = new HashSet<String>(100);
	...
	private String getValidPOS(Matcher m) {
		...
		// TODO: some pos (like idiom) may be used as a POS or as a sub section in the entry. => Check the header level.
	...
	private boolean isNymHeader(Matcher m) {
		...
		if (nym.matches())
			return nymMarkerToNymName.containsKey(nym.group(1));
		else
			return false;
	}
    ...
    private void gotoNymBlock(Matcher m) {
        ...
        Matcher nym = WikiPatterns.macroPattern.matcher(m.group(1).trim());
        if (nym.matches())
        	currentNym = nymMarkerToNymName.get(nym.group(1));
        else
        	System.err.println("WARNING: non matching nym...");
     }
	...
	private boolean isPronounciation(Matcher m) {
		...
		// if (m.group().contains("pron|")) System.out.println(m.group() + " did not match in " + wiktionaryPageName);
		return false;
	}
