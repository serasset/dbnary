...
do
  ...
  PATH=out/dbnary/$NEXT_VERSION/bin:$PATH "${SCRIPT_DIR}/../extractor/compute_diffs.sh" -m $model -f "target/extracts/$PREVIOUS_VERSION/" -t "target/extracts/$NEXT_VERSION/" -d "$DIFFS" $LANGS
done
