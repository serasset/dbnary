...
public class CompareTradsAndIWLinks extends DbnaryModel {
  ...
  public static void main(String args[]) {
    ...
    System.out.println(
        cliProg.language + " & " + nbTrans + " (" + (nbTrans / (float) nbIwlinks) * 100 + " \\%) & "
            + nbNullTrans + " (" + (nbNullTrans / (float) randomEntries.size()) * 100 + " \\%)");
  }
  ...
  private IntPair stats(String name) {
    ...
    if (iwlinks.size() > translations.size()) {
      System.out.println(name + ": " + translations.size() + "/" + iwlinks.size());
    }
