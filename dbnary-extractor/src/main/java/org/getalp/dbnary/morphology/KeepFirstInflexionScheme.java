...
public class KeepFirstInflexionScheme extends InflectionScheme {
   ...
   *
   * In a KeepFirstInflexionScheme, the feature is added only if no feature corresponding to the same
   * property is present immediately before addition.
   *
  ...
  public boolean add(MorphoSyntacticFeature morphoSyntacticFeature) {
    return super.stream().noneMatch(f -> f.property() == morphoSyntacticFeature.property()) && super.add(morphoSyntacticFeature);
  }
