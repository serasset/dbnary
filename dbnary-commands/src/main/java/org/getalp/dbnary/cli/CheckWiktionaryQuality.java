...
@Command(name = "check", mixinStandardHelpOptions = true, header = "check the mediawiki syntax of all pages of a dump.",
    description = "This command parses all standard pages using the wiki-text parser then outputs "
        ...
        + "debug messages generated by the parser and get a hint on all syntactic errors detected " + "by the parser.")
public class CheckWiktionaryQuality {
  ...
  public Integer checkStructure() throws IOException {
    structureChecker = StructureCheckerFactory.getStructureChecker(lm.getLanguage(), batch.tdbDir());
  ...
  public Integer check(BiConsumer<String, String> consumer) throws IOException {
    ...
    try {
        ...
        if (xmlr.isStartElement() && xmlr.getLocalName().equals(WiktionaryIndexer.pageTag)) {
          ...
          if (!title.equals("")) {
            ...
            if (nbRelevantPages % 1000 == 0) {
              System.err.println(
                  "Extracted: " + nbRelevantPages + " pages in: " + formatHMS(totalRelevantTime) + " / Average = " + (totalRelevantTime / nbRelevantPages)
                      + " ms/extracted page (" + (System.currentTimeMillis() - relevantTimeOfLastThousands) / 1000 + " ms) (" + nbPages + " processed Pages)");
      ...
      System.err.println("Extracted " + nbRelevantPages + " pages in: " + formatHMS(totalRelevantTime) + " (" + nbPages + " scanned Pages)");
