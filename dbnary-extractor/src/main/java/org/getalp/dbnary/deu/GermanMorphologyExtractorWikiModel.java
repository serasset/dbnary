...
import static org.getalp.dbnary.deu.GermanInflectionData.Cas.ACCUSATIF;
import static org.getalp.dbnary.deu.GermanInflectionData.Cas.DATIF;
import static org.getalp.dbnary.deu.GermanInflectionData.Cas.GENITIF;
import static org.getalp.dbnary.deu.GermanInflectionData.Cas.NOMINATIF;
...
import static org.getalp.dbnary.deu.GermanInflectionData.GNumber.PLURAL;
import static org.getalp.dbnary.deu.GermanInflectionData.GNumber.SINGULAR;
...
public class GermanMorphologyExtractorWikiModel extends GermanDBnaryWikiModel {
  ...
  public void substituteTemplateCall(String templateName,
    ...
    try {
      ...
      if (ignoredTemplates.contains(templateName)) {
        ...
        log.debug("Morphology Extraction: Toponym morphology not yet handled --in-- {}",
            this.getPageName());
        ...
        log.debug("Morphology Extraction: Nachname morphology not yet handled --in-- {}",
            this.getPageName());
  ...
  private void extractSubstantiveForms(Map<String, String> parameterMap) {
    ...
    for (Map.Entry<String, String> e : parameterMap.entrySet()) {
      ...
      if (key.contains("Singular")) {
        ...
        log.debug("no plural, neither singular in Substantiv Ubersicht: {} | {}", key,
            wdh.currentLexEntry());
      }
  ...
  private void extractAdjectiveForms(String deklinationPageName) {
    ...
    if (null == deklinationPageContent) {
      return;
    }
    if (!deklinationPageContent.contains("Deutsch")) {
      return;
    }
  ...
  private void extractVerbForms(String conjugationPage) {
    ...
    if (null == konjugationPageContent) {
      return;
    }
    if (!konjugationPageContent.contains("Deutsch")) {
      return;
    }
