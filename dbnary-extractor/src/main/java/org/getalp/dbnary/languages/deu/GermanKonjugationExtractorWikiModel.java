...
public class GermanKonjugationExtractorWikiModel extends GermanTableExtractorWikiModel {
  ...
  public GermanKonjugationExtractorWikiModel(IWiktionaryDataHandler wdh, WiktionaryPageSource wi, Locale locale, String imageBaseURL, String linkBaseURL) {
  ...
  private boolean isVerbalSection(Heading heading) {
    ...
    return heading.getContent().templates().stream()
        .anyMatch(tmpl -> tmpl.asTemplate().getName().equals("Verbkonjugation") && tmpl.asTemplate().getParsedArg("1").equals("Deutsch"));
  }
