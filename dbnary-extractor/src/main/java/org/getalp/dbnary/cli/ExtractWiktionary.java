...
public class ExtractWiktionary {
  ...
  private IWiktionaryDataHandler wdh;
    ...
    options.addOption(OptionBuilder.withLongOpt(URI_PREFIX_LONG_OPTION)
        ...
        .hasArg().withArgName("uri").create(URI_PREFIX_SHORT_OPTION));options.addOption(FOREIGN_EXTRACTION_OPTION, false, "Extract foreign Languages");
                        ...
                        if (nbnodes != wdh.nbEntries()) {
                            ...
                            if (nbRelevantPages % 1000 == 0) {
                                System.err.println("Extracted: " + nbRelevantPages + " pages in: " + totalRelevantTime + " / Average = "
                                        + (totalRelevantTime / nbRelevantPages) + " ms/extracted page (" + (System.currentTimeMillis() - relevantTimeOfLastThousands) / 1000 + " ms) ("
                                        + nbPages +" processed Pages in " + (System.currentTimeMillis() - startTime) + " ms / Average = " + (System.currentTimeMillis() - startTime) / nbPages + ")");
