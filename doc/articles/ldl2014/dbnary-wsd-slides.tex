...
Modern Greek	&	56638	&	8368	14.77\%	&	12	&	12	\\
...
%% Add the figures about glosses + numbers
...
\begin{frame}
	...
	%TikZ picture pour entourer le meilleur résultat en rouge en overlay
	        \begin{tikzpicture}
	            \node<1-3>[anchor=south west,inner sep=0] at (0,0) {\includegraphics[width=.40\textwidth]{french}\includegraphics[width=.39	\textwidth]{portuguese}};
	            \draw<2-3>[orange,thick,rounded corners] (.9,2.65) rectangle (1.2,2.95);
	            \draw<2-3>[orange,thick,rounded corners] (5.6,2.68) rectangle (5.9,2.98);
	            \node<3>[anchor=south west,inner sep=0, fill=white!20] at (0.25,0){
	        ...
	        \begin{tikzpicture}
	            \node<1-3>[anchor=south west,inner sep=0] at (0,0) {	\includegraphics[width=.40\textwidth]{finnish}};
	            \draw<2-3>[orange,thick,rounded corners] (.8,2.50) rectangle (1.1,2.80);
	        \end{tikzpicture}	        
