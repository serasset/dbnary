...
class TranslationLineParser(page: String) extends WikiRegexParsers {
  ...
  private var source: WikiCharSequence = _
  ...
  def translationLine: Parser[List[Translation]] = language ~ translations ^^ {
    case Language(name, null) ~ list =>
      ...
      val languageCode = EstonianLanguageCodes.threeLettersCode(name)
      if (null != languageCode) list.map(t => {
  ...
  def extractTranslationLine(input: WikiCharSequence, senseLocal: Boolean, delegate: WiktionaryDataHandler): Unit = {
    parseTranslationLine(input, delegate.currentPagename()).foreach {
      case Translation(lg, wr, localGloss, use) =>
        val g2 = createGloss(localGloss, delegate)
        delegate.registerTranslation(senseLocal, lg, g2, use, wr)
    }
  }
  ...
  def createGloss(localGloss: String, delegate: IWiktionaryDataHandler): Resource =
    delegate.createGlossResource(localGloss)
