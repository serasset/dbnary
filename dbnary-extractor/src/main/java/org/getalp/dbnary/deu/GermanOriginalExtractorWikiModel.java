...
public class GermanOriginalExtractorWikiModel extends DbnaryWikiModel {
  ...
  public GermanOriginalExtractorWikiModel(IWiktionaryDataHandler wdh, WiktionaryIndex wi,
    ...
    if (null != page) {
      if (!page.contains("\n")) {
        ...
        for (String key : pag.keySet()) {
          ...
          if (-1 == r.indexOf("Bild") && -1 == r.indexOf("Titel") && -1 == r.indexOf("Konjugation")
              && (-1 == r.indexOf("Deutsch") && -1 != r.indexOf(wdh.currentWiktionaryPos()))) {
            r = extractString(r, "=", "\n");
            if (!r.isEmpty()) {
              ...
              for (int e = 0; e < itemsList.getLength(); e++) {
                ...
                if (name.equals("#text")) {
                  ...
                  if (!form.isEmpty() && !form.contains("Pers.")) {
                    ...
                    int tmp =
                        change ? (j - jBegin >= 2 ? (j - jBegin) - 2 : j - jBegin) : (j - jBegin);
                    int nbr = (i - iBegin);
