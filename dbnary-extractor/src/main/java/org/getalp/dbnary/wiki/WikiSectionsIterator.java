...
public class WikiSectionsIterator implements Iterator<WikiSection> {
    ...
    public void init() {
        if (baseIterator.hasNext()) currentToken = baseIterator.next();
    }
    public boolean eof() {
    ...
    public WikiSection next() {
        if (eof()) throw new NoSuchElementException("No remaining section in this wikitext.");
        WikiText.Heading sectionHeading = (WikiText.Heading) currentToken;
        WikiText.WikiContent sectionContent = content.getWikiText().new WikiContent(currentToken.offset.end);
        advance();
